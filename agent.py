...
class AgentTools(llm.ToolContext):
    ...
    @llm.function_tool(
        description="Save booking intent after caller confirms appointment. Call this ONCE after you have name, phone, email, date, time."
    )
    async def save_booking_intent(
        self,
        start_time: Annotated[
            str, "ISO 8601 datetime e.g. '2026-03-01T10:00:00+05:30'"
        ],
...
class OutboundAssistant(Agent):
    def __init__(
        ...
        if token_count > 600:
            logger.warning(
                f"[PROMPT] Prompt exceeds 600 tokens — consider trimming for latency"
            )
...
async def entrypoint(ctx: JobContext):
    ...
    # ── OUTBOUND: If phone_number in metadata, dial the call ─────────────
    ...
    if phone_number and outbound_trunk_id:
        ...
        try:
            ...
            logger.info(
                f"[OUTBOUND] SIP participant created: {sip_participant.sip_call_id}"
            )
            # Wait for participant to join
            try:
                participant = await ctx.wait_for_participant(
                    identity=f"sip_{phone_number.replace('+', '')}"
                )
                logger.info(f"[OUTBOUND] Participant joined: {participant.identity}")
            except Exception as e:
                logger.warning(f"[OUTBOUND] Participant wait timeout: {e}")
        except api.TwirpError as e:
            sip_code = e.metadata.get("sip_status_code") if e.metadata else ""
            sip_status = e.metadata.get("sip_status") if e.metadata else ""
            logger.error(
                f"[OUTBOUND] Failed to dial: {e.message} (SIP {sip_code} {sip_status})"
            )
            ctx.shutdown()
            return
        except Exception as e:
            logger.error(f"[OUTBOUND] Error creating SIP participant: {e}")
            ctx.shutdown()
            return

    # Extract from SIP participants (for inbound calls or after outbound dial)
    ...
    if caller_history:
        ...
        live_config["agent_instructions"] = (
            live_config.get("agent_instructions", "") + caller_history
        )
    ...
    if llm_provider == "groq":
        ...
        agent_llm = openai.LLM(
            model=llm_model, max_completion_tokens=120
        )  # cap tokens (#7)
    ...
    if stt_provider == "deepgram":
        try:
            ...
            logger.warning(
                "[STT] deepgram plugin not installed — falling back to Sarvam"
            )
    ...
    if tts_provider == "sarvam":
        ...
        try:
            ...
            cartesia_voice = live_config.get("cartesia_voice_id", "") or os.getenv(
                "CARTESIA_VOICE_ID", ""
            )
            if cartesia_api_key:
                if cartesia_voice:
                    agent_tts = cartesia.TTS(
                        model="sonic-3", voice=cartesia_voice, api_key=cartesia_api_key
                    )
                    logger.info(f"[TTS] Using Cartesia — voice: {cartesia_voice}")
                else:
                    agent_tts = cartesia.TTS(model="sonic-3", api_key=cartesia_api_key)
                    logger.info("[TTS] Using Cartesia (default voice)")
            else:
    ...
    if _noise_cancel:
        try:
            room_input = RoomInputOptions(
                close_on_disconnect=False, noise_cancellation=_noise_cancel
            )
        except Exception:
            room_input = RoomInputOptions(close_on_disconnect=False)

...
app = FastAPI(title="Vogent Dashboard")
...
def read_config():
    ...
    return {
        "first_line": get_val(
            "first_line",
            "FIRST_LINE",
            "Namaste! This is Aryan from RapidX AI — we help businesses automate with AI. Hmm, may I ask what kind of business you run?",
        ),
        "agent_instructions": get_val("agent_instructions", "AGENT_INSTRUCTIONS", ""),
        "stt_min_endpointing_delay": float(
            get_val("stt_min_endpointing_delay", "STT_MIN_ENDPOINTING_DELAY", 0.6)
        ),
...
async def api_get_transcript(log_id: str):
    ...
    try:
        ...
        res = (
            supabase.table("call_logs").select("*").eq("id", log_id).single().execute()
        )
        data = res.data
        ...
        return PlainTextResponse(
            ...
            headers={
                "Content-Disposition": f"attachment; filename=transcript_{log_id}.txt"
            },
        )
    except Exception as e:
...
async def api_get_contacts():
    ...
    try:
        ...
        return sorted(
            contacts.values(), key=lambda x: x["last_seen"] or "", reverse=True
        )
    except Exception as e:
...
<head>
  ...
  <title>AI Voice Demo — Vogent</title>
  <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;500;600;700&display=swap" rel="stylesheet">
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    body{font-family:'Poppins',sans-serif;background:#ffffff;color:#000000;display:flex;align-items:center;justify-content:center;min-height:100vh;flex-direction:column;gap:24px;padding:24px}
    .card{background:#ffffff;border:1px solid #e0e0e0;border-radius:20px;padding:40px;max-width:440px;width:100%;text-align:center;box-shadow:0 4px 24px rgba(0,0,0,0.1)}
    h1{font-size:22px;font-weight:700;margin-bottom:6px}
    .sub{color:#666666;font-size:13px;margin-bottom:28px}
.avatar{width:80px;height:80px;border-radius:50%;background:linear-gradient(135deg,#333,#000);display:flex;align-items:center;justify-content:center;font-size:36px;margin:0 auto 24px}
    .btn-start{background:#000;color:#fff}
    .vol-bar span{width:4px;background:#000;border-radius:2px;transition:height 0.1s}
  </style>
</head>
<body>
  <div class="card">
    ...
    <h1>Talk to Aryan</h1>
    <div class="sub">AI-powered voice agent · Vogent</div>
...
async def api_call_bulk(request: Request):
    ...
    lk_secret = cfg.get("livekit_api_secret") or os.environ.get(
        "LIVEKIT_API_SECRET", ""
    )
    for phone in numbers:
        if not phone.startswith("+"):
            results.append(
                {"phone": phone, "status": "error", "message": "Must start with +"}
            )
            continue
        try:
...
async def api_demo_token():
    ...
    try:
        ...
        api_secret = config.get("livekit_api_secret") or os.environ.get(
            "LIVEKIT_API_SECRET", ""
        )
...
try:
    ...
    _voice_calls_total = Counter(
        "voice_calls_total", "Total calls handled by the agent"
    )
    _voice_calls_booked = Counter(
        "voice_calls_booked_total", "Calls that resulted in a booking"
    )
...
<head>
  ...
  <style>
    *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    body {{ font-family: 'Poppins', sans-serif; background: var(--bg); color: var(--text); display: flex; height: 100vh; overflow: hidden; letter-spacing: -0.02em; }}
    * {{ font-size: 102%; }}
...
<nav id="sidebar">
  <div class="sidebar-brand">
    ...
    <div>
      ...
      <div class="brand-sub">Vogent</div>
    </div>
  </div>
...
<div id="main">
  ...
  <div id="page-agent" class="page">
    ...
    <div class="section-card">
      ...
      <div class="form-group">
        ...
        <input type="text" id="first_line" value="{config.get("first_line", "")}" placeholder="Namaste! This is Aryan from Vogent...">
  ...
  <div id="page-demo" class="page">
    ...
    <div class="section-card" style="max-width:640px;margin-top:0;">
      ...
      <iframe id="demo-iframe" src="" style="width:100%;height:520px;border:none;border-radius:12px;background:#ffffff;display:none;"></iframe>
